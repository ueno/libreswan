...
static stf_status record_v2_IKE_INTERMEDIATE_request(struct ike_sa *ike,
						     struct dh_local_secret *local_secret);
static stf_status record_v2_IKE_INTEMEDIATE_response(struct ike_sa *ike,
						       ...
						       struct msg_digest *null_md)
{
	...
	if (group != NULL) {
		...
		return record_v2_IKE_INTERMEDIATE_request(ike, NULL);
	}
}
						     ...
						     struct dh_local_secret *local_secret)
{
	...
	if (ike->sa.st_v2_ike_ppk == PPK_IKE_INTERMEDIATE) {
		...
		/* Send additional KE; FIXME should it also work with PPK? */
...
stf_status process_v2_IKE_INTERMEDIATE_response_continue(struct state *st, struct msg_digest *md)
{
	...
	const struct dh_desc *group = next_additional_ke_desc(ike);
	if (group != NULL) {
		if (!calc_v2_ike_intermediate_keymat(ike, &ike->sa.st_ike_spis, HERE)) {
			/* already logged */
			return STF_FATAL;
		}
	} else {
